...
export async function POST(request: NextRequest) {
  try {
    ...
    const isFirstUser = userCount.results[0].count === 0;
    ...
    if (!isFirstUser) {
      ...
      if (adminCheck.results.length === 0 || adminCheck.results[0].role !== 'admin') {
        return NextResponse.json({ 
          success: false, 
        ...
        if (perm.client_id && perm.permission_type) {
          await db.execute(
            sql`INSERT INTO user_permissions (user_id, client_id, permission_type) 
                VALUES (${result.results[0].id}, ${perm.client_id}, ${perm.permission_type})`
          );
        }
      }
    }
...
export async function verifyAuth(request: NextRequest, requiredRole?: string, clientId?: number) {
  ...
  if (clientId && user.role !== 'admin') {
    ...
    if (permissionResult.results.length === 0) {
      return { 
        authenticated: true, 
        authorized: false,
